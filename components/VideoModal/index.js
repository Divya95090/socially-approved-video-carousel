...
const VideoPlayer = ({ video, isActive, isCommentsOpen, setIsCommentsOpen, isShareOpen, setIsShareOpen }) => {
  ...
  const shareToPlatform = (platform) => {
    ...
    if (platform === "whatsapp") {
      shareLink = `https://api.whatsapp.com/send?text=${shareText}${uniqueShareUrl}`;
    } else if (platform === "twitter") {
      shareLink = `https://twitter.com/intent/tweet?text=${shareText}&url=${uniqueShareUrl}`;
    }
            ...
            <div className="relative">
              ...
              <AnimatePresence>
                {isShareOpen && (
                  ...
                  >
                    <button 
                      onClick={(e) => { e.stopPropagation(); shareToPlatform('whatsapp'); setIsShareOpen(false); }} 
                      className="flex items-center gap-3 px-3 py-2.5 hover:bg-white/10 rounded-xl transition-colors text-white group"
                    >
                    ...
                    <button 
                      onClick={(e) => { e.stopPropagation(); shareToPlatform('twitter'); setIsShareOpen(false); }} 
                      className="flex items-center gap-3 px-3 py-2.5 hover:bg-white/10 rounded-xl transition-colors text-white group"
                    >
